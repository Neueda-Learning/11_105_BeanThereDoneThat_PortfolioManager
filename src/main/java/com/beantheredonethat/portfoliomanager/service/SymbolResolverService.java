...
public class SymbolResolverService {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(SymbolResolverService.class);
    ...
    public SymbolResolutionResult resolveSymbol(
            String symbol,
            String assetType) {
        ...
        if (normalizedAssetType == null) {
            ...
            logger.warn(
                    "Unsupported asset type '{}' for symbol '{}'",
                    assetType,
                    normalizedSymbol
            );
        ...
        /*
         * Crypto is handled by CoinGecko.
         * Do not resolve crypto through Yahoo Finance.
         *
         * Example:
         * BTC -> BTC
         * BTC -> BTC-USD ❌
         */
        if ("CRYPTO".equals(normalizedAssetType)) {
        ...
        try {
            ...
            logger.warn(
                    "Yahoo search failed for symbol '{}': {}",
                    normalizedSymbol,
                    ex.getMessage()
            );
            ...
            logger.warn(
                    "Symbol resolution failed for symbol '{}': {}",
                    normalizedSymbol,
                    ex.getMessage()
            );
    ...
    private JsonNode selectBestQuote(
            JsonNode quotes,
            String inputSymbol,
            String normalizedAssetType) {
        ...
        Set<String> expectedTypes =
                expectedQuoteTypes(normalizedAssetType);
    ...
    private int scoreCandidate(
        ...
        String input =
                inputSymbol.toUpperCase(Locale.ROOT);


        String candidate =
                candidateSymbol.toUpperCase(Locale.ROOT);
        ...
        if("STOCK".equals(normalizedAssetType)
                && (candidate.contains(".")
                || !candidate.contains("-"))) {
    ...
    private Set<String> expectedQuoteTypes(
            String normalizedAssetType) {
    ...
    private String normalizeAssetType(
            String assetType) {
    ...
    private String normalizeQuoteType(
            String quoteType) {
    ...
    private SymbolResolutionResult unresolved(
            String originalSymbol) {


        return new SymbolResolutionResult(
                normalizeSymbol(originalSymbol),
                null,
                null,
                null
        );
